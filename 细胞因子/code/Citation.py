...
class Citation(InMemoryDataset):
    ...
    def __init__(self, root, name, alpha, adj_type=None, transform=None, pre_transform=None):
        ...
        super(Citation, self).__init__(r'G:\图神经网络编程\有向异构\HAN-master\data\acm\保存', transform, pre_transform)
        self.data, self.slices = torch.load(r'G:\图神经网络编程\有向异构\HAN-master\data\acm\保存\checkpoint_model_epoch_{}.pth.tar')#self.processed_paths[0])
    ...
    def process(self):
        data, L1, L2 = citation_datasets(self.raw_dir, self.name, self.alpha, self.adj_type)
        #data = read_planetoid_data(self.raw_dir, self.name)
        data = data if self.pre_transform is None else self.pre_transform(data)
        print(data)
        filepath = os.path.join(r'G:\图神经网络编程\有向异构\HAN-master\data\acm\保存', 'checkpoint_model_epoch_{}.pth.tar')
        torch.save(self.collate([data]), filepath)#self.processed_paths[0])
        print('6666')
    def __repr__(self):
        return '{}()'.format(self.name)

#这个函数是生成邻接矩阵、特征矩阵的地方
def citation_datasets(save_path=r'G:\图神经网络编程\有向异构\HAN-master\data\acm\ACM3025.mat', dataset='ACM3025', alpha=0.1, adj_type='ib'):
...
#    g = load_npz_dataset(dataset_path)
    print('aaaaa')
    g = scio.loadmat(r'G:\图神经网络编程\有向异构\HAN-master\data\acm\ACM3025.mat')
...
#    adj, features, labels = g['A'], g['X'], g['z']
    ...
    #             dic[i] = m
    # li = []
    # for yy in dic.values():
    #     li.append(yy)
    # nli = np.array(li)

    #把邻接矩阵换成csr_matrix
    M_ = ad()
    adj_q = sp.csr_matrix(M_)

    #把特征矩阵换成csr_matrix
    M1_ = fea()
    pd.DataFrame(M1_).to_excel(r'G:\图神经网络编程\有向异构\HAN-master改\合并\特征矩阵.xlsx', index=None, header=None)
    features_q = sp.csr_matrix(M_)
    #adj, features, labels = adj_q, features_q.astype(float), nli
    adj, features = adj_q, features_q.astype(float)
    ...
    if adj_type == 'un':
        ...
        edge_index, edge_weight, L1= get_appr_directed_adj(alpha, indices, features.shape[0],features.dtype)
        #data = Data(x=features, edge_index=edge_index, edge_weight=edge_weight, y=labels)
    ...
    pd.DataFrame(L0).to_excel(r'G:\图神经网络编程\有向异构\HAN-master改\合并\结果(k=0).xlsx',index = None ,header = None)
    pd.DataFrame(L1).to_excel(r'G:\图神经网络编程\有向异构\HAN-master改\合并\结果(k=1).xlsx',index = None ,header = None)
    pd.DataFrame(L2).to_excel(r'G:\图神经网络编程\有向异构\HAN-master改\合并\结果(k=2).xlsx',index = None ,header = None)
...
if __name__ == "__main__":
    data = citation_datasets(save_path=r'G:\图神经网络编程\有向异构\HAN-master\data\acm\ACM3025.mat', dataset='ACM3025.mat')
