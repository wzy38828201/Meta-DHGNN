...
parser.add_argument('--dataset', type=str,default='ACM3025')#'cora_ml'
parser.add_argument('--gpu-no', type=int, default=0)#0
parser.add_argument('--runs', type=int, default=20)#20
...
parser.add_argument('--hidden', type=int, default=32)#32
...
class InceptionBlock(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight, edge_index2, edge_weight2):
        x0 = self.ln(x)#k=0
        x1 = self.conv1(x, edge_index, edge_weight)#k=1
        x2 = self.conv2(x, edge_index2, edge_weight2)#k=2
        return x0, x1, x2

class Sparse_Three_Sum(torch.nn.Module):#就是把0,1,2及以上邻近度的矩阵相加
...
class Sparse_Three_Concat(torch.nn.Module):#将0,1,2及以上邻近度的矩阵按照行或列拼接
    def __init__(self, dataset):
    ...
    def forward(self, data):
        ...
        x = torch.cat((x0,x1,x2),1)#1表示按列拼接,0表示按照行拼接
        x = self.ln1(x)
...
def run_digcn(dataset,gpu_no):
    ...
    print('ddd')
    log = Sparse_Three_Sum(dataset)
    val_loss, test_acc, test_std, time = run(dataset, gpu_no, log, args.runs, args.epochs, args.lr, args.weight_decay, args.early_stopping)
...
if __name__ == '__main__':
    if args.dataset is not None:
        ...
        dataset_name = ['cora_ml','citeseer','ACM3025']
