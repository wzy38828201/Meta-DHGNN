...
def get_macro_acm_roc():
    ori_fpr, ori_tpr, ori_roc_auc = macro(r'\results\original\ACM\\')
    meta_fpr, meta_tpr, meta_roc_auc = macro(r'\results\meta\ACM\\')
    adj_fpr, adj_tpr, adj_roc_auc = macro(r'\results\adj\ACM\\')
    all_fpr, all_tpr, all_roc_auc = macro(r'\results\all\ACM\\')
    print('aaaaaaaaa')
    # Plot all ROC curves
    lw=2
    plt.figure()
    ...
    plt.title('Macro-average ROC curve on the ACM dataset',fontdict=font)
    plt.legend(loc="lower left",prop=font)
    # plt.show()
    plt.savefig(r'\results\ACM_macro-average_ROC.pdf', dpi=600,bbox_inches='tight', format="pdf")

def get_macro_imdb_roc():
    ori_fpr, ori_tpr, ori_roc_auc = macro(r'G:\图神经网络编程\有向异构\dglhan_all_code20220914\results\original\IMDB\\')
    meta_fpr, meta_tpr, meta_roc_auc = macro(r'G:\图神经网络编程\有向异构\dglhan_all_code20220914\results\meta\IMDB\\')
    adj_fpr, adj_tpr, adj_roc_auc = macro(r'G:\图神经网络编程\有向异构\dglhan_all_code20220914\results\adj\IMDB\\')
    ...
    plt.title('Macro-average ROC curve on the IMDB dataset',fontdict=font)
    plt.legend(loc="lower left",prop=font)
    # plt.show()
    plt.savefig(r'\results\IMDB_macro-average_ROC.pdf', dpi=600,bbox_inches='tight', format="pdf")
