...
#
#     train_shot_0 = 30#ACM:50 imdb:30
#     #val_shot_1 = 35
#
#     select_class = [0, 1, 2]
#     class1_idx = []
...
#     #     else:
#     #         y_.append(tt)
# #    y_ = torch.from_numpy(y_.todense()).long()
#
#     r0 = []
#     r1 = []
#     r2 = []
...
#     print('aaaaaaaaaaaaaaaaaaaaaaa')
    ...
    # pd.DataFrame(labelssy).to_csv(r'G:\图神经网络编程\有向异构\dglhan_all_code20220914\results\仅adj\IMDB\labels.csv', index=False, header=False)#r'G:\图神经网络编程\有向异构\dglhan_all_code20220914\results\
    # pd.DataFrame(predictionsy).to_csv(r'G:\图神经网络编程\有向异构\dglhan_all_code20220914\results\仅adj\IMDB\prediction.csv', index=False, header=False)
    # pd.DataFrame(logitssy).to_csv(r'G:\图神经网络编程\有向异构\dglhan_all_code20220914\results\仅adj\IMDB\logits.csv', index=False, header=False)

    # #计算TN……
    # from sklearn import metrics
    # logitss1, labelss1, predictions1 = evaluate1(model, g, features, labels, test_mask, loss_fcn)
    # fpr2, tpr2, thresholds0 = metrics.roc_curve(labelss1, predictions1, pos_label=3,drop_intermediate = True)
    # print('auc:', metrics.auc(fpr2,tpr2),len(labelss1),len(predictions1))
    # cg = confusion_matrix(labelss1, predictions1)
    # tn, fp, fn, tp = cg.ravel()
    # precision1 = tp/(tp+fp)
    # Recall = tp/(tp+fn)
    # print(cg, tn, fp, fn, tp,"precision1:",precision1,"Recall:",Recall)
    # print(labelss1, "\n",predictions1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.003)#0.007
    ...
    parser.add_argument('--num_epochs', type=int, default=50)#imdb:20,ACM:50
    parser.add_argument('--num_epochs1', type=int, default=61)#acm:1,imdb:10
