...
def load_citation(dataset_str='cora', normalization='FirstOrderGCN', cuda=True):
    a = pd.read_excel('Experiments/string_interactions.xlsx', header=None)
    c = pd.read_excel('Experiments/Cytokine name.xlsx', header=None)
    a1 = a[0].tolist()
    a2 = a[1].tolist()
    a3 = a1 + a2
    a4 = a[2].tolist()
    ...
    #print(labels)
    a = pd.read_csv('Experiments/All factor categories.csv', index_col=0)
    b = a.iloc[:, 0].values
    b[:] = b[:]
