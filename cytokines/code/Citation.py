...
class Citation(InMemoryDataset):
    ...
    def __init__(self, root, name, alpha, adj_type=None, transform=None, pre_transform=None):
        ...
        super(Citation, self).__init__('save', transform, pre_transform)
        self.data, self.slices = torch.load('save/checkpoint_model_epoch_{}.pth.tar')#self.processed_paths[0])
    ...
    def process(self):
        ...
        print(data)
        filepath = os.path.join('save', 'checkpoint_model_epoch_{}.pth.tar')
...
#这个函数是生成邻接矩阵、特征矩阵的地方
def citation_datasets(save_path='data/ACM3025.mat', dataset='ACM3025', alpha=0.1, adj_type='ib'):
...
#    g = load_npz_dataset(dataset_path)
    print('aaaaa')
    g = scio.loadmat('data/ACM3025.mat')
...
#    adj, features, labels = g['A'], g['X'], g['z']
    ...
    #把特征矩阵换成csr_matrix
    M1_ = fea()
    pd.DataFrame(M1_).to_excel('feature matrix.xlsx', index=None, header=None)
    ...
    pd.DataFrame(L0).to_excel('result(k=0).xlsx',index = None ,header = None)
    pd.DataFrame(L1).to_excel('result(k=1).xlsx',index = None ,header = None)
    pd.DataFrame(L2).to_excel('result(k=2).xlsx',index = None ,header = None)
...
if __name__ == "__main__":
    data = citation_datasets(save_path='data/ACM3025.mat', dataset='ACM3025.mat')
