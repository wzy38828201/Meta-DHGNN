...
def load_cyto(remove_self_loop):
...
#    gs = [r_g]
    ...
    www1 = pd.read_excel(r'\experiments\labels00.xlsx', index=None)
    tr1 = www1[www1[2] == 1].index.tolist()
    tr2 = www1[www1[3] == 1].index.tolist()
    tr1_ = tr1[:int(len(tr1)*0.2)]
    tr2_ = tr2[int(len(tr2)*0.8):]
    tr1__ = [n1 for n1 in tr1 if n1 not in tr1_]
    tr2__ = [n1 for n1 in tr2 if n1 not in tr2_]
    tr1___ = tr1__[:int(len(tr1__)*0.5)]
    tr2___ = tr2__[:int(len(tr2__) * 0.5)]
    tr1____ = [n2 for n2 in tr1__ if n2 not in tr1___]
    tr2____ = [n2 for n2 in tr2__ if n2 not in tr2___]

    tr3 = np.array([tr1_ + tr2_])
    tr3_ = np.array([tr1___ + tr2___])
    tr3__ = np.array([tr1____ + tr2____])
    labels = www1[4]
...
def load_data(dataset, remove_self_loop=False):
    if dataset == 'CYTO':
        return load_cyto(remove_self_loop)
    elif dataset == 'ACMRaw':
