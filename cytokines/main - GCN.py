...
def main(args):
    ...
    # pd.DataFrame(labelssy).to_csv(r'G:\图神经网络编程\有向异构\dglhan改实验\结果\Meta-GNN\labels.csv', index=False, header=False)#r'G:\图神经网络编程\有向异构\dglhan_all_code20220914\results\
    # pd.DataFrame(predictionsy).to_csv(r'G:\图神经网络编程\有向异构\dglhan改实验\结果\Meta-GNN\prediction.csv', index=False, header=False)
    # pd.DataFrame(logitssy).to_csv(r'G:\图神经网络编程\有向异构\dglhan改实验\结果\Meta-GNN\logits.csv', index=False, header=False)
