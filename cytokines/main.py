...
def main(args):
    ...
    # #——————Meta——————
    # step = args['step']  #Depending on the number of metagrams you want to generate
    #
    ...
    pd.DataFrame(labelssy).to_csv(r'\results\all\labels.csv', index=False, header=False)#
    pd.DataFrame(predictionsy).to_csv(r'\results\all\prediction.csv', index=False, header=False)
    pd.DataFrame(logitssy).to_csv(r'\results\all\logits.csv', index=False, header=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_decay2', type=float, default=0.0008)
    ...
    parser.add_argument('--train_shot_0', type=int, default=15)
