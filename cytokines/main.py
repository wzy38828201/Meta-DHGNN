...
def main(args):
    ...
    pd.DataFrame(labelssy).to_csv('labels.csv', index=False, header=False)#r'G:\图神经网络编程\有向异构\dglhan_all_code20220914\results\
    pd.DataFrame(predictionsy).to_csv('prediction.csv', index=False, header=False)
    pd.DataFrame(logitssy).to_csv('logits.csv', index=False, header=False)
