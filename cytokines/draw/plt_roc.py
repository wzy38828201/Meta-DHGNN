...
def get_macro_acm_roc():
    att_fpr, att_tpr, att_roc_auc = macro('cytokines/Results/Meta-GNN')
    ori_fpr, ori_tpr, ori_roc_auc = macro('cytokines/Results/original')
    meta_fpr, meta_tpr, meta_roc_auc = macro('cytokines/Results/meta')
    adj_fpr, adj_tpr, adj_roc_auc = macro('cytokines/Results/adj')
    all_fpr, all_tpr, all_roc_auc = macro('cytokines/Results/all')
    # Plot all ROC curves
    lw=2
    plt.figure()
    ...
    # plt.show()
    plt.savefig(r'G:\图神经网络编程\有向异构\dglhan改实验\结果\细胞因子_macro-average_ROC.png', dpi=500)
    
def get_micro_acm_roc():
    att_fpr, att_tpr, att_roc_auc = micro('cytokines/Results/Meta-GNN')
    ori_fpr,ori_tpr,ori_roc_auc = micro('cytokines/Results/original')
    meta_fpr,meta_tpr,meta_roc_auc = micro('cytokines/Results/meta')
    adj_fpr,adj_tpr,adj_roc_auc = micro('cytokines/Results/adj')
    all_fpr,all_tpr,all_roc_auc = micro('cytokines/Results/all')
    ...
    plt.legend(loc="lower right")
    # plt.show()
    plt.savefig('cytokines_micro-average_ROC.png', dpi=500)
